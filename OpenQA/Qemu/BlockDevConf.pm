...
sub _push_new_drive_dev ($self, $id, $drive, $model, $num_queues = undef, $sector_size = undef) {
...
sub add_existing_drive ($self, $id, $file_name, $model, $size, %args) {
    my $num_queues = $args{num_queues};
    my $sector_size = $args{sector_size};
    ...
    return $self->_push_new_drive_dev($id, $overlay, $model, $num_queues, $sector_size);
}
