...
sub _wait_for_migrate ($self) {
    ...
    $self->_wait_while_status_is(
        qr/paused|finish-migrate/,
    ...
    $self->handle_qmp_command(
        {
            ...
            arguments => {
                ...
                up => (
                    !defined $args->{network_enabled}
                      || $args->{network_enabled} ? Mojo::JSON->true : Mojo::JSON->false
                )}
        },
        fatal => 1
    );
...
sub save_memory_dump ($self, $args) {
    ...
    $self->_migrate_to_file(
        compress_level => $compress_method eq 'internal' ? $compress_level : 0,
    ...
    $bdc->for_each_drive(
        sub {
            ...
            my $req = {
                ...
                arguments => {
                    'node-name' => $overlay->backing_file->node_name,
...
sub find_ovmf () {
    first { -e } @bmwqemu::ovmf_locations;
}
...
sub virtio_console_fifo_names () {
    map { $_ . '.in', $_ . '.out' } virtio_console_names;
}
...
sub delete_virtio_console_fifo () {
    unlink $_ or bmwqemu::fctwarn("Could not unlink $_ $!") for grep { -e } virtio_console_fifo_names;
}
...
sub setup_tpm ($self, $arch) {
    ...
    unless (-e $vmsock) {
        ...
        my @args = (
            'swtpm', 'socket', '--tpmstate', "dir=$vmpath", '--ctrl', "type=unixio,path=$vmsock", '--log', 'level=20',
            '-d'
        );
...
sub start_qemu ($self) {
    ...
    if ($vars->{UEFI_PFLASH} && (($vars->{ARCH} // '') eq 'x86_64')) {
        ...
        die
"No UEFI firmware can be found! Please specify UEFI_PFLASH_CODE/UEFI_PFLASH_VARS or BIOS or UEFI_BIOS or install an appropriate package"
          unless $vars->{UEFI_PFLASH_CODE};
    }
    if ($vars->{UEFI_PFLASH} || $vars->{BIOS}) {
        bmwqemu::fctinfo(
'UEFI_PFLASH and BIOS are deprecated. It is recommended to use UEFI_PFLASH_CODE and UEFI_PFLASH_VARS instead. These variables can be auto-discovered, try to just remove UEFI_PFLASH.'
        );
    }
    ...
    die
'HDDFORMAT has been removed. If you are using existing images in some other format then qcow2 overlays will be created on top of them'
      if $vars->{HDDFORMAT};
    ...
    }
    {
        ...
        for (my $i = 0; $i < $num_networks; $i++) {
            if ($vars->{NICTYPE} eq "user") {
                ...
                sp('netdev',
                    [qv "tap id=qanet$i ifname=$tapdev[$i] script=$tapscript[$i] downscript=$tapdownscript[$i]"]);
            }
    ...
    if ($vars->{NICTYPE} eq "tap") {
        ...
        $self->{proc}->_process->on(
            collected => sub {
                $self->{proc}->_process->emit('cleanup')
                  unless exists $self->{stop_only_qemu} && $self->{stop_only_qemu} == 1;
            });
            ...
            cleanup => sub {
                eval {
                    for (my $i = 0; $i < $self->{allocated_networks}; $i++) {
                        $self->_dbus_call(
                            'unset_vlan',
                            (@{$self->{allocated_tap_devices}})[$i],
                            (@{$self->{allocated_vlan_tags}})[$i]);
                    }
                }
            });
...
sub cont_vm ($self, @) {
    $self->update_request_interval(delete $self->{_qemu_saved_request_interval})
      if $self->{_qemu_saved_request_interval};
