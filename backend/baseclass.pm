...
sub do_capture_tick ($self, $buckets = undef, $timeout = undef, $starttime = undef) {
...
sub matching_needles_and_tags ($mustmatch) {
    my $needles = [];
    my @tags;
...
sub set_tags_to_assert ($self, $args) {
    ...
    my ($needles, $tags) = matching_needles_and_tags($mustmatch);
    {    # remove duplicates
        my %h = map { $_ => 1 } @$tags;
        @$tags = sort keys %h;
    }
