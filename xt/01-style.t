...
is qx{git grep -l -e 'sub \\S\\+ [^(]\\+' --and --not -e 'sub [(\{]' --and --not -e 'sub \\S\\+(' backend/ consoles/},
  '', 'All backends and consoles use sub signatures everywhere (nameless still allowed)';
...
is qx{git grep -I -l -P '^use (warnings|strict)' ':!external/'}, '',
  'No files using "warning|strict", should use Mojo::Base instead';
is qx{git grep -I -l 'sub [a-z_A-Z0-9]\\+()'}, '',
