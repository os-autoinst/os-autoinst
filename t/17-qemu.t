...
use constant TMPPATH => '/tmp/17-qemu.t/';
...
@cmdl = qw(qemu-kvm -static-args
  ...
  -device scsi-cd,id=cd0-device,drive=cd0-overlay0);
%vars = (NUMDISKS => 1,
    HDDMODEL       => 'scsi-hd',
    CDMODEL        => 'scsi-cd',
...
path('/tmp/17-qemu.t/new-drive-and-cdrom.json')->spurt($proc->serialise_state);
...
@cmdl = qw(qemu-kvm -static-args
  ...
  -device scsi-cd,id=cd0-device,drive=cd0-overlay1
  -incoming defer);
