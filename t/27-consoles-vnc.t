...
subtest 'login on real VNC server via vnctest, request and receive frame buffer' => sub {
    ...
    note "running Xvnc for display $display (port $port) and connect via $bmwqemu::topdir/scripts/vnctest";
    my $xvnc_pid = open(my $xvnc_pipe, "Xvnc -depth 16 -SecurityTypes None -ac :$display 2>&1 |");
    my $vnc_test_pid = open(my $vnc_test_pipe, "$bmwqemu::topdir/scripts/vnctest --port $port --verbose 2>&1 |");
