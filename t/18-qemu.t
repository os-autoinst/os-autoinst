...
@cmdl = ([qw(create -f qcow2  raid/hd1 10G)], [qw(create -f qcow2  raid/hd2 12G)]);
@gcmdl = $bdc->gen_qemu_img_cmdlines();
is_deeply(\@gcmdl, \@cmdl, 'Generate qemu-img command line for multiple new drives');

@cmdl = (
    '-blockdev', 'driver=file,node-name=hd1-overlay0-file,filename=raid/hd1-overlay0,cache.no-flush=on',
...
$proc = OpenQA::Qemu::Proc->new()->_static_params(['-foo'])->qemu_bin('qemu-kvm');
...
path($path)->spurt($proc->serialise_state());
$proc = OpenQA::Qemu::Proc->new()->_static_params(['-static-args'])->qemu_bin('qemu-kvm')->qemu_img_bin('qemu-img');
$proc->deserialise_state(path($path)->slurp());
@gcmdl = $proc->gen_cmdline();
...
@cmdl = (
    [qw(create -f qcow2 -F qcow2 -b raid/hd0 raid/hd0-overlay1 10G)],
...
path($path)->spurt($proc->serialise_state());
$proc = OpenQA::Qemu::Proc->new()->_static_params(['-static-args'])->qemu_bin('qemu-kvm');
$proc->deserialise_state(path($path)->slurp());
$ssc = $proc->snapshot_conf;
$bdc = $proc->blockdev_conf;
...
sub qemu_proc ($static_params, $vars) {
    return OpenQA::Qemu::Proc->new()->_static_params([$static_params])->qemu_bin('qemu-kvm')->qemu_img_bin('qemu-img')
      ->configure_controllers($vars)->configure_blockdevs('disk', 'raid', $vars)->configure_pflash(\%vars);
}
...
subtest 'non-existing-iso' => sub {
    ...
    my $err;
    my @warnings = warnings {
        eval { $proc = qemu_proc('-static-args', \%vars); };
        $err = $@;
    };
...
subtest 'qemu was killed due to the system being out of memory' => sub {
    ...
    is(
        $state->{msg},
        'QEMU was killed due to the system being out of memory',
        'qemu was killed and the reason was shown correctly'
    );
    unlink("./Core-7.2.iso");
};
