...
subtest status => sub {
    ...
    is_deeply(
        $last_received_msg_by_fd[$answer_fd],
        {
            tags => [qw(foo bar)],
            running => 'welcome',
...
subtest 'set pause at test' => sub {
    ...
    is_deeply(
        $last_received_msg_by_fd[$cmd_srv_fd],
        {set_pause_at_test => 'some test'},
    ...
    stderr_like {
        $command_handler->process_command($answer_fd,
            {cmd => 'set_current_test', name => 'some test', full_name => 'some test'})
    }
    qr/pausing test execution.*some test/, 'pausing logged';
    is $command_handler->reason_for_pause, 'reached module some test',
      'reason for pause set when reaching module to pause on';
