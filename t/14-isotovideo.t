...
subtest 'get the version number' => sub {
    ...
    chdir('/');
    combined_like { system $^X, "$toplevel_dir/isotovideo", '--version' } qr/Current version is.+\[interface v[0-9]+\]/,
      'version printed';
...
subtest 'standalone isotovideo without vars.json file and only command line parameters' => sub {
    ...
    combined_like { isotovideo(opts => "casedir=$data_dir/tests schedule=foo,bar/baz _exit_after_schedule=1") }
    qr{scheduling.+(foo|bar/baz)}, 'requested modules scheduled';
};
...
subtest 'isotovideo with git refspec specified' => sub {
    ...
    qr/Checking.*local.*deadbeef/, 'refspec in local git repository would be checked out';
};
...
subtest 'productdir variable relative/absolute' => sub {
    ...
    qr/\d* scheduling.*shutdown/, 'schedule has been evaluated';
    ...
    combined_like { isotovideo(opts => "casedir=$data_dir/tests _exit_after_schedule=1 productdir=product/foo") }
    qr/\d* scheduling.*shutdown/, 'schedule can still be found';
    ...
    my $log = combined_from {
        isotovideo(opts => "casedir=$data_dir/tests _exit_after_schedule=1 productdir=product integration_tests=1")
    };
...
subtest 'upload assets on demand even in failed jobs' => sub {
    ...
    my $log = combined_from {
        isotovideo(
            opts =>
"casedir=$data_dir/tests schedule=$module force_publish_hdd_1=foo.qcow2 qemu_no_kvm=1 arch=i386 backend=qemu qemu=i386",
            exit_code => 0
        )
    };
...
subtest 'load test success when casedir and productdir are relative path' => sub {
    ...
    my $module = 'tests/failing_module';
    my $log = combined_from {
        isotovideo(opts => "casedir=my_cases productdir=my_cases/products/foo schedule=$module", exit_code => 0)
    };
...
subtest 'upload the asset even in an incomplete job' => sub {
    ...
    ok(!-e $pool_dir . '/assets_public/publish_test.qcow2',
        'the asset defined by PUBLISH_HDD_X would not be generated in an incomplete job');
};
