...
Additionally the AI generated os-autoinst deepwiki can provide a good overview
about the complete project but without a guarantee for correctness:
https://deepwiki.com/os-autoinst/os-autoinst
...
to list all available targets. One especially useful target for developers is
`make setup-hooks` which installs git pre-commit hooks to check commit styles.
