...
has [qw(execute code write_stream process_id read_stream error_stream channel_in channel_out exit_status pidfile)];
has blocking_stop         => 0;
...
sub _open {
    ...
    # Defered removal of pidfile
    $self->on(stop => sub { unlink($self->pidfile) if $self->pidfile; });
...
sub stop {
    ...
    if ($not_dead && $self->blocking_stop) {
        ...
        push(@{$self->error}, "Could not kill process");
    }
    else {
