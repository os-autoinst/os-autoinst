...
sub connect_remote ($self, $args) {
    ...
    my $ttyconn
      = $self->backend->new_ssh_connection(hostname => $hostname, password => $args->{password}, username => 'root');
    ...
    if (!$chan->exec('smart_agetty hvc0')) {
        bmwqemu::fctwarn(
            'Unable to execute "smart_agetty hvc0" at this point: ' . ($ttyconn->error // 'unknown SSH error'));
    }
    ...
    if (!$serialchan->exec("iucvconn $zvmguest lnxhvc0")) {
        bmwqemu::fctwarn(
            'ssh iucvconn: unable to grab serial console at this point: ' . ($ssh->error // 'unknown SSH error'));
    }
    $ssh->blocking(0);
}
