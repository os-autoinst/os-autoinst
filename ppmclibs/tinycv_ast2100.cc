...
static void
dec_makehuff(struct dec_hufftbl *hu, unsigned char *hufflen)
{
...
static void
idct(int *in, int *out, PREC *quant, int max)
{
...
static void
idctqtab(unsigned char *qin, PREC *qout, double scale)
{
  int i, j;
...
static void
setpalette(int *co, int cy, int cb, int cr)
{
  co[0] = cy - 128;
  co[1] = cb - 128;
  co[2] = cr - 128;
...
void
decode_ast2100(cv::Mat *pic, const unsigned char *data, size_t datal)
{
  struct in ins, *in;
  int mcux = 0, mcuy = 0;
  int odc[3];
