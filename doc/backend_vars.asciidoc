...
_HIDE_SECRETS_REGEX;string;;If set any test variables matching the specified regular expression additional to the default '^_SECRET_' and '_PASSWORD' are excluded from being saved into vars.json or further processing. This is intended for custom test variable keys that are to be treated as secret but can not match the default patterns.
