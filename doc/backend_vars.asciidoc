...
_IN_POST_FAIL_HOOK;boolean;0;Set during `post_fail_hook` execution, allows for additional behavior in routines that are also executed as part of the test flow, e.g teardown that are called during post_fail_hook and during post_run_hooks or simply during normal test execution.
