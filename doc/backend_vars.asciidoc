...
QEMU_VIRTIO_KEYBOARD;boolean;1;Enable virtio-keyboard. The virtio-keyboard is a paravirtualized device with no emulation, higher performance and for example allows the use of `kbdrate` in SUTs to disable (or tweak) key repetition to prevent mistyping errors.
