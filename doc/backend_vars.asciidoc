...
QEMU_MULTIFD_CHANNELS;integer;2;Number of channels used to migrate data in parallel. This is the same number that the number of sockets used for migration. Can be set from 1 to 255.
QEMU_USE_MULTIFD_FOR_MIGRATION;boolean;0;Force use of multifd for migration. Usefull only for qemu < 9.1.0, since it is the mandatory for qemu >= 9.1.0.
